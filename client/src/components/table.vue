...
.ctable {
  ...
  table {
    ...
    &.info {
      ...
      th {
        background: #edf6fd;
        border-bottom: 1px solid rgba(65, 155, 249, 0.5);
      }
      td {
        border-bottom: 1px solid rgba(193, 218, 247, 0.5);
        &.check {
          border-bottom: none;
        }
      }
...
export default {
  ...
  methods: {
    ...
    filterChange(th, op) {
      if (op) {
        this.$emit('filter-change', th.filteData.filters.filter(obj => obj.select), th.prop)
      } else {
